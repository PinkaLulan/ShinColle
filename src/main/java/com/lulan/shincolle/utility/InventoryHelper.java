...
public class InventoryHelper
{
    ...
    /**
     * check target stack is same with temp stacks
     */
    ...
    public static boolean checkRingInBaubles(EntityPlayer player)
    {
      ...
      public static boolean tryFillContainer(CapaInventoryExtend inv, FluidStack fs)
      {
          ...
          if (amountMoved > 0 && inv.getHost() != null)
          {
              ...
              if (inv.getHost().getShipType() == ID.ShipIconType.TRANSPORT)
              {
    ...
    /**
     * get itemhandler from target object
     */
    public static IItemHandler getItemHandler(Object target)
    {
        if (target instanceof ICapabilityProvider)
        {
            return ((ICapabilityProvider) target).getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null);
        }
        else if (target instanceof IInventory)
        {
            return new InvWrapper((IInventory) target);
        }

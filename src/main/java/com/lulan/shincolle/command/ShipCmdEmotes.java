 ...
 *      
 */
public class ShipCmdEmotes extends BasicShipCommand {
	...
	public List addTabCompletionOptions(ICommandSender sender, String[] cmd) {
		return getListOfStringsMatchingLastWord(cmd, EmoNameArray);
	}
	...
	public void processCommand(ICommandSender sender, String[] cmd) {
		...
		if(!world.isRemote) {
			...
			if(cmd != null && cmd.length > 0) {
				type = nameToEmoID(cmd[0]);
			}
			//random emotes
			else {
				type = world.rand.nextInt(EntityFXEmotion.EMO_NUMBER + 1);
			}
			
			//get position
			float px = sender.getPlayerCoordinates().posX;
			float py = sender.getPlayerCoordinates().posY;
			float pz = sender.getPlayerCoordinates().posZ;
			float height = 1F;
			int entityType = 0;  //0:none, 1:EntityLivingBase
			...
			if(sender instanceof Entity) {
				TargetPoint point = new TargetPoint(world.provider.dimensionId, px, py, pz, 64D);
		      	CommonProxy.channelP.sendToAllAround(new S2CSpawnParticle((Entity) sender, 36, height, entityType, type), point);
			}
			else {
				TargetPoint point = new TargetPoint(world.provider.dimensionId, px, py, pz, 64D);
		      	CommonProxy.channelP.sendToAllAround(new S2CSpawnParticle(36, px, py, pz, height, entityType, type), point);
			}
		}//end server side
	}

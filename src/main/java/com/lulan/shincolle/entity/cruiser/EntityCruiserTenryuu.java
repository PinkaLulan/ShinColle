...
public class EntityCruiserTenryuu extends BasicEntityShipSmall
{
	...
	public void onLivingUpdate()
	{
		...
		if (this.world.isRemote)
		{
			...
			{
//				ParticleHelper.spawnAttackParticleAtEntity(this, 0.5D, 1D, 1D, (byte)13); //TODO debug
			}
		}
		//server side
		else
	...
	private void attackEntityWithSkill()
	{
		if (this.StateEmotion[ID.S.Phase] > 1)
		{
			...
			if (this.StateTimer[ID.T.AttackTime3] < 5 && this.StateTimer[ID.T.AttackTime3] >= 0)
			{
				...
				//attack on colliding
				if ((this.ticksExisted & 1) == 0) this.damageNearbyEntity();
        		        ...
        		        if (target.canBePushed())
        		        {
        		        	...
        		        	else
        		        	{
                 			...
                 			//for other player, send ship state for display
                 	  		CommonProxy.channelE.sendToAllAround(new S2CEntitySync(target, 0, S2CEntitySync.PID.SyncEntity_Motion), point);
        		        }
        	        }
        		}//end not same owner
        	}//end can collide
	...
	//find random safe pos
	private BlockPos findRandomSafePos(Entity target)
	{
		...
		while (loops > 0)
		{
			...
			posoffset = CalcHelper.rotateXZByAxis(6F, 0F, this.rand.nextFloat() * 360F * Values.N.DIV_PI_180, 1F);
			newpos = pos.add((int)posoffset[1], 0, (int)posoffset[0]);
			IBlockState state = this.world.getBlockState(newpos);
			
			if (BlockHelper.checkBlockSafe(state))
			{
				//return new pos
				return newpos;
			}
	...
	//find random safe pos
	private BlockPos findTopSafePos(Entity target)
	{
		BlockPos pos = new BlockPos(target);
		BlockPos newpos;
		
		//get random pos
		int loops = (int)target.height + 6;
		float[] posoffset;
	...
	{
		//get random pos
		BlockPos pos = this.findRandomSafePos(target);
	...
	{
		//get random pos
		BlockPos pos = this.findTopSafePos(target);
		Vec3d vecpos = new Vec3d(pos.getX()+0.5D, pos.getY(), pos.getZ()+0.5D);
		double dist = this.getDistanceSqToCenter(pos);
	...
	@Override
  	public void applyParticleAtAttacker(int type, Entity target, float[] vec)
  	{
  		...
  		case 2:  //heavy cannon
			...
			CommonProxy.channelP.sendToAllAround(new S2CSpawnParticle(this, 11, 0.6D, 1D, 1D), point);
			CommonProxy.channelP.sendToAllAround(new S2CSpawnParticle(this, 12, 0.6D, 0.9D, 1D), point);
		break;
	...
	@Override
  	public void applyParticleAtTarget(int type, Entity target, float[] vec)
  	{
  		...
  		case 5:  //high speed movement
  			CommonProxy.channelP.sendToAllAround(new S2CSpawnParticle(this, 44, posX, posY, posZ, vec[0], vec[1], vec[2], true), point);
		break;
		default: //melee
